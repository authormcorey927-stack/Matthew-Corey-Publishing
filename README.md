# Matthew Corey Publishing

Welcome to the central hub for all of Matthew Corey’s books, merch, and digital projects.

This repository contains:
- 📚 Pages and assets for published and upcoming books
- 🎨 Merch assets and links
- 🌐 Source files for the author website
- 🛠️ Supporting materials for reader resources and landing pages

## Contents

- `/site` — Website source files  
- `/books` — Book landing pages, summaries, sample chapters  
- `/merch` — Merchandise graphics and links  
- `/downloads` — Free resources for readers

## About Matthew Corey

Matthew Corey is an author and creator building original stories and branded worlds for readers and fans.

---

*This README may grow as the project evolves.*
Central hub for Matthew Corey’s books, merchandise, and digital publishing projects.

This repository will house:
- Author website files and landing pages
- Book descriptions, sample content, and release pages
- Merchandise links and media assets
- Free downloads and reader resources

This repo is intended to grow alongside current and future publishing projects.
